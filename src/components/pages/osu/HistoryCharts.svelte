 ...
 * 历史数据曲线
 *
 * 组件自己从 /api/osu 取数（与「最好成绩」一样是按需加载的面板），
 * 外层用 client:visible 挂载，所以第一次切到这个 tab 才会发请求。
 *
...
import { loadOsuData } from "./osu-data";
...
onMount(async () => {
	try {
		history = (await loadOsuData()).history;
		loaded = true;
	} catch (e) {
		console.warn("osu 历史数据获取失败:", e);
		error = true;
	}
});
